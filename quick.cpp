...
class bintree {
  ...
  public:
    ...
    //static vector<bintree *> searchNeighbors(bintree* indext);
    static float searchFirstLeaf(bintree* indextpp, bool right);
...
{
  cout << "quickSelect2D" << endl;
    cout << "left " << p << "right " << r << " k  " << k << endl;
    if ( p == r ) return input[p][n]; 
    int j = partition2D(input, p, r, n);
    int length = j - p +1;
    if ( length == k ) return input[j][n];//cout << "med "<<  input[j][n] << endl;
...
bintree::create2DBST(float **array, int depth, int nbreserves, int left, int right, bintree* prevt)
{
  ...
  cout << "hello 2" << "left" <<left<< "right -1" << right -1 << "med" << (right-left)/2 +1<< endl;
  float med = quickSelect2D(array, left, right-1,(right-left)/2, n);

  //quickSort2D(array, left, right -1, n);
  read(array, 10);

  cout << "med " << med << " left "<< left << " right " << right << endl;
  if (left == right) { 
    return nullptr; 
  }
  
  if( left == right - 1 ) {
     cout <<"returned" << array[left][n] << " left " << left << endl;
    return new bintree(array[left][n], prevt);
  }
  int medind = (left + right) / 2;
  t->root = med;
  cout << "med index " << medind << " med " << med << " left "<< left << " right " << right << endl;

  cout << endl << endl << endl;
  if(prevt != nullptr) t->prev = prevt;
  tl = create2DBST(array, depth, medind, left, medind, t);
  tr = create2DBST(array, depth, right, medind + 1, right, t);
  t->left = tl;
  t->right = tr;
...
// float bintree::searchFirstLeaf(bintree* indextpp){
//   cout <<"indextpp " <<  indextpp->root << endl;
//   if(indextpp->left)
//   {
//   if(indextpp->left->left== nullptr && indextpp->left->right==nullptr) return indextpp->left;
//   else 
//   }
//   if(indextpp->right && indextpp->right->left== nullptr && indextpp->right->right==nullptr) return indextpp->right;
// return 0;
// }
...
//   // else if(right) 
//   // else {
//   //   cout << "n'a pas de frere feuille"<<endl;
//   //   //searchNeighbors(indext->prev);
//   // }
